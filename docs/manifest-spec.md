...
The desktop launcher uses `Exec=openntx run <app-id> --notify`. Runtime execution remains a not-implemented backend, and run-plan diagnostics are written under `~/.local/state/openntx/logs/`.
